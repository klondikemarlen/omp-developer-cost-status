...
export function parsePluginConfig(value: unknown): PluginConfig | undefined {
  ...
  if (typeof value.settings !== "object" || value.settings === null || Array.isArray(value.settings)) {
    throw new Error("Project Time config settings must be an object.")
  }

  const settings: PluginSettingsByName = {}
  for (const [pluginName, pluginSettings] of Object.entries(value.settings)) {
    if (typeof pluginSettings !== "object" || pluginSettings === null || Array.isArray(pluginSettings)) {
